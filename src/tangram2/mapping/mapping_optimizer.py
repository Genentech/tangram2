...
class Mapper:
    ...
    def __init__(
        ...
        if wandb_config.get("project_name") is None:
            wandb_config["project_name"] = ut.random_tag()

        self.wandb_config = wandb_config

        self.S = torch.tensor(S, device=device, dtype=torch.float32)
        self.G = torch.tensor(G, device=device, dtype=torch.float32)
        self.G_norm = self.G.sum(dim=1) / self.G.sum()
        self.G_sqrt = torch.sqrt(self.G)

        assert (alpha > 0) and (alpha < 1), "alpha must be within range (0,1)"
        ...
        if adata_map is None:
            if self.random_state:
                ...
                ut.solid_random_seed(self.random_state)
        ...
        self.M_his = torch.tensor(
            np.random.normal(0, 1, (S.shape[0], G.shape[0])),
            device=device,
            dtype=torch.float32,
        )
    ...
    def _loss_fn(self, verbose=True):
        ...
        if self.coeff is None:
            ...
            M_probs = softmax(self.M, dim=1) * softmax(self.coeff)[:, None]
        ...
        # expression_term = self.lambda_g1 * cosine_similarity(torch.sqrt(G_pred), torch.sqrt(self.G), dim=1).sum()

        BC = cosine_similarity(torch.sqrt(G_pred), self.G_sqrt, dim=1)
        vg_reg = torch.mul(self.G_norm, BC).sum()
        expression_term = self.lambda_g1 * vg_reg

        entropy_reg = (torch.log(M_probs) * M_probs).sum()
        regularizer_term = self.lambda_r * entropy_reg

        # main_loss = (expression_term / self.lambda_g1).tolist()
        ...
        if verbose:
            term_numbers = [vg_reg, kl_reg, entropy_reg]
            term_names = ["Score", "KL reg", "Entropy reg"]

            d = dict(zip(term_names, term_numbers))
            clean_dict = {k: d[k] for k in d}
            msg = []
            for k in clean_dict:
    ...
    def train(
        ...
        if self.coeff is None:
            ...
            optimizer = torch.optim.Adam([self.M, self.coeff], lr=learning_rate)

        if print_each:
            logging.info(f"Printing scores every {print_each} epochs.")

        # ugly but saves a few if/else statements
        keys = [
            "total_loss",
            "main_loss",
            "vg_reg",
        ...
        training_history = {key: [] for key in keys}


        if self.wandb_log: 

            run = wandb.init(
                project=self.wandb_config.get("project_name"),
                ...
                config=dict(
                    ...
                    lambda_g1=self.lambda_g1,
                    lambda_g2=self.lambda_g2,
        ...
        for t in range(num_epochs):
            ...
            loss = run_loss["total_loss"]

            for key in keys:
                training_history[key].append(run_loss.get(key))
            ...
            if self.wandb_log:
                with torch.no_grad():
                    delta = torch.abs(self.M_his - self.M)
                    av_abs_diff = delta.mean()
                    mx_abs_diff = delta.max()
                    map_dict = {
                        "av_abs_diff(M_current,M_av)": av_abs_diff,
                        "max_abs_diff(M_current,M_av)": mx_abs_diff,
                    }

                    self.M_his = self.alpha * self.M + (1 - self.alpha) * self.M_his
                    run_loss.update(map_dict)
        ...
        with torch.no_grad():
            if self.coeff is None:
                ...
                output = softmax(self.M, dim=1).cpu().numpy()
                coefficient = (softmax(self.coeff)).cpu().numpy()
                return output, coefficient, training_history
...
class MapperConstrained:
    ...
    def _loss_fn(self, verbose=True):
        ...
        expression_term = (
            self.lambda_g1
            * cosine_similarity(torch.sqrt(G_pred), torch.sqrt(self.G), dim=1).mean()
        )
