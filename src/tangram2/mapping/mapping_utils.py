...
def adata_to_cluster_expression(
    ...
    Args:
        ...
        return_label_order (bool): If True n_obs sized vector with indices corresponding to the order of the cell type index of each observation will be returned. Default is False.
    ...
    """
    try:
        value_counts_norm = adata.obs[cluster_label].value_counts(normalize=True)
        value_counts = adata.obs[cluster_label].value_counts()
    ...
    if add_density:
        adata_ret.obs["cluster_density"] = adata_ret.obs[cluster_label].map(
            lambda i: value_counts_norm[i]
        )

    if add_counts:
        adata_ret.obs["cell_counts_per_cluster"] = adata_ret.obs[cluster_label].map(
    ...
    if return_label_order:
        ...
        label_order = np.array([label_map[x] for x in adata.obs[cluster_label].values])
        return adata_ret, label_order, unique_labels
    return adata_ret
...
def map_cells_to_space(
    ...
    if mode == "hejin_workflow":
        ...
        if isinstance(S_full,spmatrix):
            S_full = np.array(S_full.toarray())
        S_full = S_full.astype(np.float32)

        single_cell_obs = adata_sc.obs.copy()
        n_cells = adata_sc.shape[0]
        single_cell_obs["int_index"] = np.arange(n_cells)

        single_cell_var = adata_sc.var.copy()
        n_genes = adata_sc.shape[1]
        single_cell_var["int_index"] = np.arange(n_genes)
        training_genes_idx = single_cell_var.loc[training_genes]["int_index"].values
        
        adata_sc, label_order, uni_labels = adata_to_cluster_expression(
            adata_sc,
            cluster_label,
            scale=False,
            add_density=True,
    ...
    if isinstance(adata_sc.X, spmatrix):
        S = np.array(
    ...
    if mode == "cells":
        ...
        mapping_matrix, coefficient, training_history = mapper.train(
            learning_rate=learning_rate,
            num_epochs=num_epochs,
            print_each=print_each,
        )
    ...
    # Hejin Worklfow
        ...
        logging.info(
            "Begin training with {} genes and {} density_prior in clusters mode...".format(
                len(training_genes), d_str
            )
        )
        ...
        # 2. Correct single cell data

        logging.info("Renormalizing Single cell data")

        # get cell type counts, order same as `w`
        ct_counts = np.array(adata_sc.obs["cell_counts_per_cluster"].values)
        # `label_order` is returned from `adata_to_cluster_expression`
        S_full = S_full / ct_counts[label_order][:,None] * n_cells * w[label_order][:,None]
        # for consistency with older versions and torch compatibility
        S = S_full[:, training_genes_idx].astype(np.float32)
        
        
        # 3. Map with corrected single cell data 

        logging.info(
            "Begin training with {} genes and {} density_prior in cells mode after renormalization".format(
                len(training_genes), d_str
            )
        )
        ...
        # 4. save corrected single cell data 
        adata_sc_corrected = sc.AnnData(
            X = S_full.astype(np.float32), 
            obs = single_cell_obs, 
            var = single_cell_var
        )
