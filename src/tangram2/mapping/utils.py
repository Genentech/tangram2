...
from . import plot_utils as plt_ut
...
def random_tag():
    ...
    out_string = ''
    for ii in range(3):
        out_string += ''.join(random.choice(ss) for x in range(3)) + '-'
    return out_string[0:-1]


def solid_random_seed(seed: int, use_deterministic: bool = False)->None:
    """
    Sets random seed properly for numpy
    and torch (CPU and GPU)

    """
    print('Set Solid Seed')
    random.seed(seed)
    ...
    if use_deterministic:
        # from: https://docs.nvidia.com/cuda/cublas/index.html#cublasApi_reproducibility
        os.environ['CUBLAS_WORKSPACE_CONFIG'] = ':16:8'
        # from: https://pytorch.org/docs/stable/notes/randomness.html
        t.use_deterministic_algorithms(True)

def log_function(x, A, B, C):
    y = np.log(A*x+B)/np.log(C)
    return y
...
def compare_spatial_geneexp(adata_ge, adata_sp, adata_sc=None, genes=None):
    ...
    for v1, v2 in zip(X_1.T, X_2.T):
        norm_sq = np.linalg.norm(np.sqrt(v1)) * np.linalg.norm(np.sqrt(v2))
        cos_sims.append((np.sqrt(v1) @ np.sqrt(v2)) / norm_sq)
...
def cross_val(
    ...
    for train_genes, test_genes in tqdm(
        ...
        #plt_ut.plot_cell_annotation(adata_map, adata_sp, annotation='subclass_label', nrows=5, ncols=4, robust=True, perc=0.05)
        #plt_ut.plot_training_scores(adata_map, bins=10, alpha=.5)
...
def eval_metric(df_all_genes, test_genes=None):
    ...
    # tm metric
    # Fit polynomial'
    ys = list(test_gene_scores)
    xs = list(test_gene_sparsity_sp)
    #ys_trans = [x - 1 for x in ys]
    #ys_trans = [i / j for i, j in zip(ys_trans, xs)]
    ...
    #parameters, covariance = curve_fit(log_function, xs, ys)
    pol_xs = np.linspace(0, 1, 50)  # x linearly spaced
    ...
    #roots = pol.r #find the root to make polynomial equation = 0
    #root = None
    ...
    np.append(pol_xs, 1)
    np.append(pol_ys, log_function(1, parameters[0], parameters[1], parameters[2]))
    ...
    auc_test_score = np.real(auc(pol_xs, pol_ys))   #This 'auc' is a sklearn metric to calculate the area under curve
